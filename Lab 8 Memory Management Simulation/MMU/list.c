...
void list_add_ascending_by_address(list_t *l, block_t *newblk) {
    ...
    // If list is empty or new block should be placed at the front
    // because it has a smaller start address than the head.
...
void list_add_ascending_by_blocksize(list_t *l, block_t *newblk) {
    ...
    // If list is empty, just insert at head
    if (l->head == NULL) {
        newNode->next = NULL;
        l->head = newNode;
        return;
    ...
    // We want ascending order by block size.
    // This means smaller blocks come first, larger blocks go later.
    while (current != NULL) {
    ...
    // If we reach here, new block is larger or equal than all encountered, so place at the end
    prev->next = newNode;
    newNode->next = NULL;
}
...
void list_coalese_nodes(list_t *l) {
    // This function assumes the list is sorted by address in ascending order.
    if (l->head == NULL || l->head->next == NULL)
        return; // no need to coalesce
    ...
    while (current != NULL) {
        ...
        // If prev.END + 1 == current.START, merge them
        if (prev->blk->end + 1 == current->blk->start) {
            // Merge
            prev->blk->end = current->blk->end; // extend prev block
            // Remove current node
